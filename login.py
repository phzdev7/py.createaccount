...
if email_duvida.lower() == "s":
    ...
    while True:
        ...
        if "@gmail.com" in email:
            ...
            print(
                "EMAIL INCOMPREENDIDO! VOCÊ PRECISA COLOCAR @gmail.com"
            )  # ATE TER "@gmail.com" NAO VAI PARAR DE REPETIR

    senha = input(
        "digite sua senha: "
    )  # QUANDO FALAMOS SIM, SOMOS DIRECIONADOS PARA COLOCARMOS O EMAIL E A SENHA
    print(f"OLÁ, O USER TEM O EMAIL: {email} E A SENHA: {senha}")
    sys.exit()
elif (
    email_duvida.lower() == "n"
):  # .lower()   faz indentificar mesmo se a resposta for maiuscula ou minuscula
    ...
    if criar.lower() == "s":
        ...
        if idade > 150:
            print("Voce deve ter cometido um erro\n de digitação tente novamente")
            # apague a linha abaixo
            print(
                " TODO:Adicionar uma forma de voltar caso ocorra um erro\n\n: Nao tem como voltar daqui, vai avancar direto para o email"
            )
            # Nao tem como voltar aqui, vai avancar direto para o email
        elif idade >= 18:
